...
- Give every time in the reader's timezone, named (e.g. "25 Sep 2026, 14:29 AEST"), never bare UTC. LM returns epochs, so add their offset before formatting (in jq: `(.startEpoch + OFFSET_SECONDS) | todate`, and write the zone yourself, because todate always prints Z). Two ways that goes wrong, both of which have produced answers out by hours: some fields are epoch MILLISECONDS (SDTs among them), so divide by 1000 first or you will land in 1970 or 56000; and a `...OnLocal` field is already a formatted local time, so adding an offset to it shifts it again. Print an `...OnLocal` string as it stands, and say which zone it is: it is the PORTAL's, which may be neither yours nor the reader's (PortalInfo gives that zone). When both are available, an `...OnLocal` field and the epoch should agree -- if they do not, you have converted one of them twice. For anything upcoming or recent, add "in about N days" or "N days ago".
