...
- `tools/elm-ask/run.sh`: a launcher that asks which model and effort to use, then starts the container. It checks `ANTHROPIC_API_KEY` and the profile before starting, takes `--model/--effort/--profile/--port`, `--build` to build the image first and `--dry-run` to print the docker command instead of running it, and prompts only for what was not given and only when there is a terminal to ask at. Model and effort are what a question costs, and both change per run without a rebuild.
