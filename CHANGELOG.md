...
- `tools/elm-module-updates.py`: `--sort age|impact|name` picks the row order within each section — `age` (default) keeps the existing most-out-of-date-first ordering, `risk` puts the highest score first, `name` is alphabetical. `--sort risk` turns `--devices` on by itself when the lookups fit inside `--max-device-calls`, since the score is only trustworthy with a device count, and says so on stderr when they do not fit rather than silently ranking on half the picture. Impact sorting falls back to age for ties, which comes free from sorting a list that is already in age order with a stable sort; a module's blast radius says nothing about how far behind it is, so the two orderings answer different questions and both are worth having.
