...
- `tools/elm-ask` hid elm's warnings whenever any rows came back, so a query asking for a field that does not exist looked like missing data: asked for uptime on a device it requested `-f uptime`, got a row without it, and answered that uptime was not available (suggesting MetricsSummary and MetricsUsage, which are licence counts). elm had said "Warning: unknown field: uptime" all along. Warnings now come back with every result, the prompt says a warning means the query is wrong rather than the data missing, and the DeviceList notes record that device uptime is `upTimeInSeconds`, with the datasource route for uptime over time.
- `tools/elm-ask` wrote summaries naming things that were not in its own table: asked which collectors were down, it produced a correct 27-row table and then a "top 5 most critical" listing three collectors that were not down at all, and said 12 of 27 had no backup where the data says 21. The prompt now requires every name and number around a table to be taken from that table with jq -- sorted, sliced, counted -- rather than read off or recalled, and says why: the reader cannot tell the difference.
...
- `tools/elm-check-access.sh`: sends one small request per command and reports whether a profile's API token is allowed to run it (`ok`, `denied by LM (403)`, `needs id`), then prints an `allowed_commands` line holding just the commands that answered. A profile has two limits that can disagree — `allowed_commands` in its `.ini` and the LogicMonitor role behind its token — and lining them up was guesswork: elm-ask offered `CollectorList` because the example profile lists it, while the role refused it with a 403. `--all` checks every command elm has, which is a quick way to see what a token can reach at all, and `--json` gives each command with the API path it calls and its status, for lining results up with LogicMonitor role areas. `ai.example.ini` records what a role covering resources, websites and SDTs but not the settings areas actually allowed: seven of its commands returned 403, including the two worth widening a role for (CollectorList, and DatasourceList, which finding alerts by collection method depends on).
- `tools/elm-ask`: a **Markdown** button on each answer, showing the whole thing -- question, text, tables as Markdown tables, and the queries behind it, with the profile, model and time -- to copy into a ticket or wiki page, or download as `.md`. (It replaced a Save as PDF button, which went through the browser's print dialog: Markdown is what these answers actually get pasted into.) The CSV download stays for spreadsheet work, and the page still prints sensibly from the browser menu.
- `tools/elm-ask`: the credit names elm's version ("Powered by elm v1.10.0"), and the examples include a metric read ("How much memory is DEVICE using?"). That one needs a device name, so clicking it fills the box and selects DEVICE instead of asking straight away; the other examples still ask immediately.
- `tools/elm-ask`: the footer links the portal name to its LogicMonitor URL and credits elm with a link; the subtitle is generic again now the portal is named below.
- `tools/elm-ask`: `describe_command` describes commands the profile withholds as well as the ones it allows -- what a command *would* return is documentation, not portal data, and running it stays refused. Asked when a user last logged in, it can now say "AdminList carries lastLoginOn, and this profile does not allow it" instead of guessing that the API has no such thing, or naming the wrong command. The prompt also says it cannot see elm's configuration or profiles, after it described a config file it had never read.
- `tools/elm-ask`: `find_commands` also names the matching commands the profile withholds, so a refusal can be specific. Asked when someone last logged in, it had said the API does not expose user activity -- it does (`AdminList` has `lastLoginOn`), the profile just does not allow it. It can now say which command would answer, leaving the person to decide whether to allow it.
- `tools/elm-ask`: the page says which portal it is talking to -- "Ask about the <portal> portal", and in the footer -- read from the profile's `account_name` (the id and key in that file are never read). The footer's "N allowed commands" is a button that lists them, with a note that anything else is refused before a request is sent.
- `tools/elm-ask`: the Markdown and CSV buttons sit together on each answer as small labelled icons, rather than CSV hiding in each table's header; with several tables, CSV saves one file per table.
- `tools/elm-ask`: the "elm ask" title is a link back to a fresh page.
