...
- `tools/elm-module-updates.py`: `--sort age|impact|name` picks the row order within each section — `age` (default) keeps the existing most-out-of-date-first ordering, `impact` puts the widest blast radius first, `name` is alphabetical. Impact sorting falls back to age for ties, which comes free from sorting a list that is already in age order with a stable sort; a module's blast radius says nothing about how far behind it is, so the two orderings answer different questions and both are worth having.

- `tools/elm-module-updates.py`: an **impact** column scoring blast radius 0-10 — if this upgrade goes wrong, how much is wrong. Instances and devices are not equivalent inputs, so they are not weighted equally: both are log-scaled and breadth counts about twice depth, putting 1 instance on 1000 devices (6.9) above 1000 instances on 1 device (4.0). Breadth is the multiplier on the worst outcome LogicMonitor documents — an AppliesTo change that stops a module applying destroys history *per device* — and on alert storms, where 1000 hosts alerting is an incident and 1000 instances on one host is a noisy host. Depth is the volume of history at stake on that host, so it carries half the weight rather than none. Breadth uses devices actually collecting where known, falling back to devices applied; without `--devices` there is no device count, so the score reflects instances alone and the legend says it understates wide, shallow modules. The inputs stay in their own columns so the score is auditable, and the two coefficients are a one-line change.
